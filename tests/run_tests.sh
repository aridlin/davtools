...
cleanup() {
    ...
    rm -f base64dec.txt.b64.txt base64dec.txt.dec.bin empty_b64.txt.b64.txt empty_b64.txt.dec.bin
...
echo "Testing base64dec..."
printf "YWJj" > base64dec.txt.b64.txt
if put_file base64dec.txt.b64.txt http://127.0.0.1:8081/convert/base64dec/in/base64dec.txt.b64.txt &&
   get_file http://127.0.0.1:8081/convert/base64dec/out/base64dec.txt.dec.bin base64dec.txt.dec.bin; then
    check_file base64dec.txt.dec.bin
    check_content base64dec.txt.dec.bin "abc"
else
    fail "base64dec conversion request"
fi

echo "Testing base64dec empty..."
printf " " > empty_b64.txt.b64.txt
if put_file empty_b64.txt.b64.txt http://127.0.0.1:8081/convert/base64dec/in/empty_b64.txt.b64.txt &&
   get_file http://127.0.0.1:8081/convert/base64dec/out/empty_b64.txt.dec.bin empty_b64.txt.dec.bin; then
    if [ -f empty_b64.txt.dec.bin ] && [ "$(stat -c%s empty_b64.txt.dec.bin)" -eq 0 ]; then
        pass "empty_b64.txt.dec.bin created and is empty"
    else
        fail "empty_b64.txt.dec.bin not empty or missing"
    fi
else
    fail "base64dec empty conversion request"
fi
