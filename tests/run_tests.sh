...
cleanup() {
    ...
    rm -f base64_dec.txt base64_dec.txt.bin base64_dec_empty.txt base64_dec_empty.txt.bin test_json.json test_json.min.json server_test.log
}
trap cleanup EXIT
...
if put_file base64_dec.txt http://127.0.0.1:8081/convert/base64-dec/in/base64_dec.txt &&
   get_file http://127.0.0.1:8081/convert/base64-dec/out/base64_dec.bin base64_dec.txt.bin; then
    check_file base64_dec.txt.bin
    check_content base64_dec.txt.bin "abc"
else
    fail "base64-dec conversion request"
fi

echo "Testing base64-dec (empty)..."
printf " " > base64_dec_empty.txt
if put_file base64_dec_empty.txt http://127.0.0.1:8081/convert/base64-dec/in/base64_dec_empty.txt &&
   get_file http://127.0.0.1:8081/convert/base64-dec/out/base64_dec_empty.bin base64_dec_empty.txt.bin; then
    if [ -f "base64_dec_empty.txt.bin" ] && [ "$(stat -c%s "base64_dec_empty.txt.bin")" -eq 0 ]; then
        pass "base64_dec_empty.txt.bin created and empty"
    else
        fail "base64_dec_empty.txt.bin not created or not empty"
    fi
else
