...
echo '{ "key": "value with \"quotes\"", "array" : [ 1, 2, 3 ] }' > test.json
...
if grep -Fq '{"key":"value with \"quotes\"","array":[1,2,3]}' test.min.json; then
    echo "SUCCESS: json-min matched expected output"
else
    echo "FAILED: json-min did not match expected output"
    cat test.min.json
    exit 1
fi
