...
cleanup() {
    ...
    rm -f clean.txt clean.png tiny.png tiny.jpg invert.png img.gif test.pdf pdf.png test.mp4 mp4.gif base64.txt base64.txt.b64.txt server_test.log test.json test.min.json empty_trigger.json empty_trigger.min.json
}
trap cleanup EXIT
...
if ! grep -Fq '{"key":"value","num":123}' test.min.json; then
    echo "FAILED: test.min.json content mismatch"
    cat test.min.json
    exit 1
fi

echo "Testing json-min empty..."
printf " " > empty_trigger.json
