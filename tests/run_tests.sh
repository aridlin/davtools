...
cleanup() {
    ...
    rm -f test_json.json test_json.min.json test_csv.csv test_csv.json test_empty_csv.csv test_empty_csv.json server_test.log
}
trap cleanup EXIT
...
echo "Testing csv-json (empty input)..."
printf " " > test_empty_csv.csv
if put_file test_empty_csv.csv http://127.0.0.1:8081/convert/csv-json/in/test_empty_csv.csv &&
   get_file http://127.0.0.1:8081/convert/csv-json/out/test_empty_csv.json test_empty_csv.json; then
    check_file test_empty_csv.json
    check_content test_empty_csv.json $'[\n  [" "]\n]'
else
    fail "csv-json conversion request (empty input)"
fi
