...
grep -Fq '{"key":"value","array":[1,2,3]}' test.min.json || { echo "FAILED: json-min content mismatch"; exit 1; }

echo "Testing json-min (empty output trigger)..."
...
grep -Fq '' empty_trigger.min.json || { echo "FAILED: json-min empty trigger content mismatch"; exit 1; }
