...
cleanup() {
    ...
    rm -f test_csv.csv test_csv.json test_csv_empty.csv test_csv_empty.json
}
trap cleanup EXIT
...
if put_file test_csv.csv http://127.0.0.1:8081/convert/csv-json/in/test_csv.csv &&
   get_file http://127.0.0.1:8081/convert/csv-json/out/test_csv.json test_csv.json; then
    ...
    check_content test_csv.json '[{"name":"Alice","age":"30"},{"name":"Bob","age":"25"}]'
else
    fail "csv-json conversion request"
fi

echo "Testing csv-json (empty)..."
printf " " > test_csv_empty.csv
if put_file test_csv_empty.csv http://127.0.0.1:8081/convert/csv-json/in/test_csv_empty.csv &&
   get_file http://127.0.0.1:8081/convert/csv-json/out/test_csv_empty.json test_csv_empty.json; then
    check_file test_csv_empty.json
    check_content test_csv_empty.json '[]'
else
    fail "csv-json (empty) conversion request"
fi
