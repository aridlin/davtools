...
void test_one(const std::string& op, bool disable_broken) {
    try {
        if (op == "png-jpg") {
            ...
            if (out.empty() || out[0].data.empty()) throw std::runtime_error("empty output");
            if (out[0].data != "abc") {
                throw std::runtime_error("base64-dec mismatch");
            }
            // test padding 1
            auto out2 = g_registry.at(op).fn("selftest.txt", {'Y', 'W', 'I', '='});
            if (out2.empty() || out2[0].data.empty()) throw std::runtime_error("empty output padding 1");
            if (out2[0].data != "ab") {
                throw std::runtime_error("base64-dec mismatch padding 1");
            }
            // test padding 2
            auto out3 = g_registry.at(op).fn("selftest.txt", {'Y', 'Q', '=', '='});
            if (out3.empty() || out3[0].data.empty()) throw std::runtime_error("empty output padding 2");
            if (out3[0].data != "a") {
                throw std::runtime_error("base64-dec mismatch padding 2");
            }
        } else if (op == "json-min") {
