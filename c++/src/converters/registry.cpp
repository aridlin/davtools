...
void test_one(const std::string& op, bool disable_broken) {
    try {
        if (op == "png-jpg") {
            ...
            auto out = g_registry.at(op).fn("selftest.b64", {'Y', 'W', 'J', 'j'}); // abc
            if (out.empty() || out[0].data.empty()) throw std::runtime_error("empty output");
            if (out[0].data != "abc") {
                throw std::runtime_error("base64-dec mismatch on 0 padding");
            }
            auto out1 = g_registry.at(op).fn("selftest.b64", {'Y', 'W', 'I', '='}); // ab
            if (out1.empty() || out1[0].data.empty()) throw std::runtime_error("empty output");
            if (out1[0].data != "ab") {
                throw std::runtime_error("base64-dec mismatch on 1 padding");
            }
            auto out2 = g_registry.at(op).fn("selftest.b64", {'Y', 'Q', '=', '='}); // a
            if (out2.empty() || out2[0].data.empty()) throw std::runtime_error("empty output");
            if (out2[0].data != "a") {
                throw std::runtime_error("base64-dec mismatch on 2 padding");
            }
            auto out_nl = g_registry.at(op).fn("selftest.b64", {'Y', 'W', 'J', 'j', '\n'}); // abc\n
            if (out_nl.empty() || out_nl[0].data.empty()) throw std::runtime_error("empty output");
            if (out_nl[0].data != "abc") {
                throw std::runtime_error("base64-dec mismatch with newline");
            }
        } else if (op == "json-min") {
            const char* in = "{ \n  \"key\" : \"value with spaces\" \t }";
            std::vector<std::uint8_t> input(in, in + std::strlen(in));
            auto out = g_registry.at(op).fn("selftest.json", input);
            if (out.empty() || out[0].data.empty()) throw std::runtime_error("empty output");
...
std::vector<std::string> canonical_ops_for_testing() {
    return {"png-jpg", "invert", "img-gif", "pdf-png", "mp4-gif", "virustest", "md5", "sha256", "base64", "base64-dec", "json-min"};
}
