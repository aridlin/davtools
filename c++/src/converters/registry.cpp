...
void test_one(const std::string& op, bool disable_broken) {
    try {
        if (op == "png-jpg") {
            ...
            // Test 0 padding
            auto out0 = g_registry.at(op).fn("selftest.b64", {'Y', 'W', 'J', 'j', '\n'});
            if (out0.empty() || out0[0].data.empty()) throw std::runtime_error("empty output");
            if (out0[0].data != "abc") {
                throw std::runtime_error("base64-dec decode mismatch for 0 padding");
            }

            // Test 1 padding
            auto out1 = g_registry.at(op).fn("selftest.b64", {'Y', 'W', 'I', '=', '\n'});
            if (out1.empty() || out1[0].data.empty()) throw std::runtime_error("empty output");
            if (out1[0].data != "ab") {
                throw std::runtime_error("base64-dec decode mismatch for 1 padding");
            }

            // Test 2 padding
            auto out2 = g_registry.at(op).fn("selftest.b64", {'Y', 'Q', '=', '=', '\n'});
            if (out2.empty() || out2[0].data.empty()) throw std::runtime_error("empty output");
            if (out2[0].data != "a") {
                throw std::runtime_error("base64-dec decode mismatch for 2 padding");
            }
        } else if (op == "json-min") {
