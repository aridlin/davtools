...
void test_one(const std::string& op, bool disable_broken) {
    try {
        if (op == "png-jpg") {
            ...
            auto check_decode = [&](const std::string& in_str, const std::string& expected) {
                std::vector<std::uint8_t> in(in_str.begin(), in_str.end());
                auto out = g_registry.at(op).fn("selftest.txt", in);
                if (out.empty() || out[0].data != expected) {
                    throw std::runtime_error("base64-decode mismatch for input: " + in_str);
                }
            };
            check_decode("", "");
            check_decode("YQ==", "a");       // 2 bytes padding
            check_decode("YWI=", "ab");      // 1 byte padding
            check_decode("YWJj", "abc");     // 0 bytes padding
            check_decode("YW J\nj", "abc");  // With spacing
        } else if (op == "json-min") {
