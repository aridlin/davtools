...
std::vector<OutputArtifact> convert_base64dec(const std::string&, const std::vector<std::uint8_t>&);
...
void init_registry_once_locked() {
    ...
    g_registry.emplace("base64dec", Entry{convert_base64dec, true, {}});
...
void test_one(const std::string& op, bool disable_broken) {
    try {
        ...
        } else if (op == "base64dec") {
            auto out = g_registry.at(op).fn("selftest.txt.b64.txt", {'Y', 'W', 'J', 'j', '\n'});
            if (out.empty() || out[0].data.empty()) throw std::runtime_error("empty output");
            if (out[0].data != "abc") {
                throw std::runtime_error("base64dec mismatch");
            }
        } else if (op == "json-min") {
...
std::vector<std::string> canonical_ops_for_testing() {
    return {"png-jpg", "invert", "img-gif", "pdf-png", "mp4-gif", "virustest", "md5", "sha256", "base64", "base64dec", "json-min"};
}
