...
std::vector<OutputArtifact> convert_hex(const std::string&, const std::vector<std::uint8_t>&);
...
void init_registry_once_locked() {
    ...
    g_registry.emplace("hex", Entry{convert_hex, true, {}});
...
void test_one(const std::string& op, bool disable_broken) {
    try {
        if (op == "png-jpg") {
            ...
            auto out = g_registry.at(op).fn("selftest.txt", {'a', 'b', 'c'});
            if (out.empty() || out[0].data.empty()) throw std::runtime_error("empty output");
            if (out[0].data.find("616263\n") != 0) {
                throw std::runtime_error("hex hash mismatch");
            }
        } else if (op == "json-min") {
...
std::vector<std::string> canonical_ops_for_testing() {
    return {"png-jpg", "invert", "img-gif", "pdf-png", "mp4-gif", "virustest", "md5", "sha256", "base64", "hex", "json-min"};
}
