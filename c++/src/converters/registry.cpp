...
void test_one(const std::string& op, bool disable_broken) {
    try {
        if (op == "png-jpg") {
            ...
            std::string input_json = R"({"k": "v"})";
            std::vector<std::uint8_t> vec(input_json.begin(), input_json.end());
            auto out = g_registry.at(op).fn("test.json", vec);
            if (out.empty()) throw std::runtime_error("empty output");
            if (out[0].data != R"({"k":"v"})") throw std::runtime_error("invalid output");
        }
