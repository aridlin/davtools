...
void test_one(const std::string& op, bool disable_broken) {
    try {
        if (op == "png-jpg") {
            ...
            auto out = g_registry.at(op).fn("selftest.json", {input.begin(), input.end()});
            if (out.empty() || out[0].data.empty()) throw std::runtime_error("empty output");
            if (out[0].data != "{\"a\":1}\n") {
                throw std::runtime_error("json-min output mismatch");
            }
            // test invalid json string (unclosed quote)
            try {
                g_registry.at(op).fn("selftest.json", {'"'});
                throw std::runtime_error("json-min failed to throw on invalid input");
            } catch (const std::runtime_error& err) {
                if (std::string(err.what()).find("Unclosed string") == std::string::npos) {
                    throw; // Re-throw if it wasn't the expected exception
                }
            }
        }
