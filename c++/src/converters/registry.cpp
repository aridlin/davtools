...
void test_one(const std::string& op, bool disable_broken) {
    try {
        if (op == "png-jpg") {
            ...
            // invalid json
            try {
                std::vector<std::uint8_t> bad = {'{', '"', 'a', '"', ':', ' ', '"', 'b'};
                g_registry.at(op).fn("selftest.json", bad);
                throw std::runtime_error("expected exception for invalid json");
            } catch (const std::runtime_error& err) {
                if (std::string(err.what()) != "invalid JSON: unclosed string") {
                    throw;
                }
            }
            // valid json
            std::vector<std::uint8_t> good = {'{', ' ', '\n', '\t', '"', 'a', '"', ' ', ':', ' ', '1', ' ', '}'};
            auto out = g_registry.at(op).fn("selftest.json", good);
