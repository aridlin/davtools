...
std::vector<std::pair<std::string, std::vector<int>>> preview_fields(std::string_view op) {
    ...
    if (op == "halftone") return {{"density", range(1, 100)}, {"size", range(2, 32)}};
...
std::string settings_preview(std::string_view op, const std::string& name, ConverterOptions options) {
    ...
    const auto key = name == "source.png" ? "source" : std::string(op) + ':' + std::to_string(options.threshold_percent) + ':' +
        std::to_string(options.halftone_density) + ':' + std::to_string(options.halftone_size) + ':' + std::to_string(options.bayer_grid);
    std::scoped_lock lock(mutex);
    if (auto it = cache.find(key); it != cache.end()) return it->second;
    std::string png;
    if (name == "source.png") {
        png.assign(reinterpret_cast<const char*>(source_image().data()), source_image().size());
